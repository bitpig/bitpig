...
#if defined(NDEBUG)
# error "Bitpig cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x32b503d0002671fcbe5f4b22406e735f5aa638605b94bc5fb0de47e80f66bdb8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitpig: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bitpig Signed Message:\n";
...
{
    // Bitpig: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
   ...
   if(nHeight == 100)  
   {
       nSubsidy = 125000100 * COIN;
   }

    // Subsidy is cut in half every 2000000 blocks, which will occur approximately every 11 years
    nSubsidy >>= (nHeight / 2000000); // Bitpig: 2 Million blocks in ~11 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Bitpig: 2 days
static const int64 nTargetSpacing = 3 * 60; // Bitpig: 3 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitpig: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitpig: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x32b503d0002671fcbe5f4b22406e735f5aa638605b94bc5fb0de47e80f66bdb8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Thursday 1 December 2016 The BiTPiG is born";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043d29c6a1bb3f6b7c424c6885ccb7305ab4d73b4b264b1d16b05847dd4b2f264d0a8a36d4a9c5ed134a6ca6c96b1310d89c49e20e4555b1c825dcf3e6690d8038") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1480554000;
        ...
        assert(block.hashMerkleRoot == uint256("0xc342bc6101f9d6912816d128e2311324b6343cc59109a70db80265d762b5e078"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Bitpig: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitpigMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitpigMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitpigMiner(CWallet *pwallet)
{
    printf("BitpigMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitpig-miner");
    ...
    try { loop {
        ...
        printf("Running BitpigMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitpigMiner, pwallet));
}
